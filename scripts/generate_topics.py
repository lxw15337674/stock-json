...
def collect_all_topics(stocks_dir: Path) -> Tuple[Dict[str, Dict], Dict[str, Set[str]], Dict[str, Path]]:
    """
    收集所有股票文件中的题材信息
    
    Returns:
        (主题材信息字典, 附带题材到主题材的映射, 股票名称到文件路径的映射)
        - 主题材信息: {主题材名: {'stocks': [股票列表], 'comment': 注释, 'attached': [附带题材列表]}}
        - 附带题材映射: {附带题材名: {主题材集合}}
        - 股票路径映射: {股票名称: 文件路径（相对于stocks_dir）}
    """
    # {主题材名: {'stocks': {股票集合}, 'comment': 注释, 'attached': [附带题材列表]}}
    main_topics = defaultdict(lambda: {'stocks': set(), 'comment': '', 'attached': []})
        ...
        try:
            ...
            for main_topic, info in topics_dict.items():
                ...
                for attached_topic in info['attached']:
                    if attached_topic:
                        main_topics[main_topic]['attached'].append(attached_topic)
    ...
    # 转换 stocks 为列表并去重 attached
    for topic in main_topics:
        main_topics[topic]['stocks'] = sorted(list(main_topics[topic]['stocks']))
        main_topics[topic]['attached'] = sorted(list(set(main_topics[topic]['attached'])))
    
    return dict(main_topics), dict(attached_to_main), stock_to_path
...
def generate_attached_topic_file(attached_name: str, main_topics: Set[str], output_file: Path):
    """
    生成附带题材 MD 文件，链接到相关主题材
...
def generate_topic_files(main_topics: Dict[str, Dict], 
    ...
    Args:
        main_topics: {主题材名: {'stocks': [股票列表], 'comment': 注释, 'attached': [附带题材列表]}}
    ...
    for attached_name, main_topic_set in attached_to_main.items():
        ...
        generate_attached_topic_file(attached_name, main_topic_set, output_file)
        print(f"生成附带题材文件: {output_file} (链接到 {len(main_topic_set)} 个主题材)")
...
def main():
    ...
    print("开始收集题材信息...")
    main_topics, attached_to_main, stock_to_path = collect_all_topics(stocks_dir)
    
    print(f"找到 {len(main_topics)} 个主题材")
    print(f"找到 {len(attached_to_main)} 个附带题材")
    
    print("\n开始生成题材文件...")
    generate_topic_files(main_topics, attached_to_main, stock_to_path, output_dir)
