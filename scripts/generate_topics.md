...
`generate_topics.py` 是一个用于从股票 Markdown 文件中提取题材信息并生成题材索引文件的脚本。它会自动扫描 `stocks` 目录下的所有股票文件，提取题材和附带题材信息，然后在 `topics` 目录下生成对应的索引文件。
...
**股票链接：**
- 从 `topics/主题材/` 到 `stocks/...` 的相对路径：`../../stocks/...`
- 锚点指向三级标题：`#主题材名称`

**附带题材链接：**
- 从 `topics/主题材/` 到 `topics/附带题材/` 的相对路径：`../附带题材/...`
- 从 `topics/附带题材/` 到 `topics/主题材/` 的相对路径：`../主题材/...`
...
开始生成题材文件...
生成主题材文件: topics/主题材/羽绒.md (包含 1 个股票)
生成附带题材文件: topics/附带题材/冬季.md (链接到 1 个主题材, 1 个股票)
生成附带题材文件: topics/附带题材/寒潮.md (链接到 1 个主题材, 1 个股票)

完成！所有文件已生成到 topics 目录
```
